...
import FormularioComponenteEdicion from "@/Components/formUpdateOccupation";
import TablePostulation from "@/Components/tablaPostulation";
...
export default function TablaOccupations({ occupations, setOccupations, onOccupationSeleccionado }) {
  ...
  const columns = [
          ...
          onClick={async () => {
            ...
            if (confirm.isConfirmed) {
              try {
                ...
                Swal.fire({ icon: 'success', title: 'Deleted!', text: 'Occupation has been deleted.', timer: 2000, showConfirmButton: false });
              } catch (error) {
                Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to delete occupation.' });
              }
            }
          }}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-red-600" fill="none"
            viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round"
    ...
    },
    {
      ...
      cell: row => (
              ...
              html: (
                <FormularioComponenteEdicion
                  ...
                  onSuccess={(updatedOccupation) => {
                    ...
                    Swal.fire({ icon: 'success', title: 'Updated!', text: 'Occupation updated successfully.', timer: 2000, showConfirmButton: false });
                  }}
                />
              ),
              showConfirmButton: false,
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-yellow-500" fill="none"
            viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536M9 11l6-6 
            3.536 3.536-6 6H9v-3.536z" />
  ...
  const handleRowClick = async (row) => {
    if (selectedRow?.id === row.id) {
      setSelectedRow(null);
      onOccupationSeleccionado?.(null);
    } else {
      setSelectedRow(row);
      try {
        const response = await axios.get('', { params: { idOccupation: row.id } });
        onOccupationSeleccionado?.(response.data);
      } catch (error) {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-lg w-full flex flex-col">
      <div className="flex justify-between items-center border-b-4 border-red-700 pb-2">
        <h2 className="text-xl font-bold text-red-800">Occupations</h2>
        <button
